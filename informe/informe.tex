...
%TODO: releer y acortar partes inncesarias y cambiar el fraseo.
%NOTE: SOLO DEBE IR LA DESCRIPCION BREVE DE LAS PARTES DEL SISTEMA, no acerca de la implementacion
El sistema de recomendación difuso propuesto contiene 5 módulos para su correcto funcionamiento:

\begin{enumerate}
	\item \textbf{Lector de datasets}: La primera etapa en el análisis de sentimiento es la lectura e
	      interpretación del dataset. Este debe contener, como mínimo, dos columnas clave: el texto de los
	      tweets y una etiqueta que indique el sentimiento asociado, siendo 1 para positivo y 0 para negativo.

	\item \textbf{Lexicón de Sentimientos}: Un lexicon es un diccionario de palabras que categoriza los
	      sentimientos de cada palabras y les asigna un valor exacto (crisp). En este trabajo se e utiliza el
	      lexicón de sentimientos de VADER(Valence Aware Dictionary and sentiment Reasoner). Este modelo es
	      ampliamente utilizado debido a su capacidad para manejar datos informales como los tweets.

	\item \textbf{Fuzzificador}: Su propósito es traducir valores numéricos exactos(crisp) en grados de
	      pertenencia difusos. La fuzzificación se aplica a las métricas de sentimiento obtenidas del lexicón de
	      VADER, resultando en conjuntos difusos que representan el nivel del sentimiento. Para realizar la
	      fuzzificación utilizaremos funciones de membresía triangular.

    % TODO: mejorar fraseo de esto y ampliar
    \item \textbf{Sistema de inferencia}: este modulo se encarga de transformar los conjuntos entradas
        mencionadas se les aplica un sistema de inferencia difusa.
...
%-------------------------------        Metodologia        -------------------------------
\section{Metodologia}
El proceso del analisis de sentimiento propuesto consta de 4 pasos esenciales, en los cuales se utilizan los
5 modulos previamente mencionados. Estos pasos son:

\begin{itemize}
	\item Lectura del dataset
	\item Fuzzificacion
	\item Inferencia
	\item Defuzzificacion
\end{itemize}

A continuación, describimos cada uno de estos pasos en detalle.
