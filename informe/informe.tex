...
\begin{figure}[ht]
	...
	\resizebox{\linewidth}{!}{ % Ajusta el tamaño al ancho de la página
		\begin{tikzpicture}[node distance=2cm]
			% Nodes
			\node (fetch) [process] {Obtener Tweet};
			\node (lexicon) [process, right of=fetch, xshift=1cm] {Verificar Léxico};
			\node (compute) [process, right of=lexicon, xshift=1cm] {Calcular Puntaje};
			\node (mamdani) [process, below of=compute, yshift=0cm] {Aplicar Reglas Mamdani};
			\node (aggregate) [process, left of=mamdani, xshift=-1.5cm] {Agregar Resultados};
			\node (result) [process, left of=aggregate, xshift=-1cm] {Resultado Final};
			% Arrows
			\draw [arrow] (fetch.east) -- (lexicon.west);
			\draw [arrow] (lexicon.east) -- (compute.west);
			\draw [arrow] (compute.south) -- (mamdani.north);
			\draw [arrow] (mamdani.west) -- (aggregate.east);
			\draw [arrow] (aggregate.west) -- (result.east);
		\end{tikzpicture}
	}
